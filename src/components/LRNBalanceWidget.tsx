...
function Skeleton({ size }: { size: WidgetSize }) {
  return (
    <>
      ...
      {size === "sm" && (
        <span className="lrn-skeleton lrn-skeleton-sm" aria-label="Loading LRN balance" />
      )}
      {size === "md" && (
      ...
      {size === "lg" && (
        <div className="lrn-skeleton-card lrn-skeleton-card-lg" aria-label="Loading LRN balance">
...
function SmWidget({
  ...
  return (
    <Tooltip>
      <span className="lrn-sm-pill" role="status" aria-label={`${animatedBalance} LRN tokens`}>
        <span className="lrn-trophy" aria-hidden="true">🏆</span>
        <span className="lrn-sm-balance">{animatedBalance.toLocaleString()}</span>
        <span className="lrn-sm-label">LRN</span>
        <span className="lrn-tooltip">
          LearnTokens (LRN) are your on-chain proof of learning.
          Earn them by completing course milestones. They unlock
          scholarships, governance rights, and your reputation score.
        </span>
      </span>
  ...
  return (
    <div className="lrn-md-card" role="region" aria-label="LRN Token Balance">
      <div className="lrn-md-header">
        <span className="lrn-md-icon" aria-hidden="true">🏆</span>
        <span className="lrn-md-title">LearnTokens</span>
        <Tooltip>
          <span className="lrn-info-btn" tabIndex={0} aria-label="What are LearnTokens?">?
            <span className="lrn-tooltip lrn-tooltip-left">
              LearnTokens (LRN) are soulbound reputation tokens earned by
              completing course milestones. They unlock scholarships,
              governance voting, and your on-chain profile rank.
            </span>
          </span>
        </Tooltip>
      </div>
      ...
      {gained && (
        <div className="lrn-md-change" aria-label={`Gained ${delta} LRN`}>
          <span className="lrn-change-arrow">↑</span>
          +{delta} LRN
        </div>
      )}
  ...
  return (
    <div className="lrn-lg-card" role="region" aria-label="LRN Token Profile">
      ...
      <div className="lrn-lg-header">
        <span className="lrn-lg-icon" aria-hidden="true">🏆</span>
        <div>
          <p className="lrn-lg-title">LearnToken Balance</p>
          <p className="lrn-lg-sub">Soulbound Reputation Score</p>
        </div>
        <Tooltip>
          <span className="lrn-info-btn lrn-info-btn-sm" tabIndex={0} aria-label="What are LearnTokens?">?
            <span className="lrn-tooltip lrn-tooltip-left">
              LearnTokens (LRN) are non-transferable proof of learning,
              minted on-chain when you complete verified course milestones.
              Your balance determines scholarship eligibility and
              governance power within the LearnVault DAO.
            </span>
          </span>
        </Tooltip>
      </div>
      ...
      <div className="lrn-lg-balance" aria-live="polite">
        <span className="lrn-lg-number">{animatedBalance.toLocaleString()}</span>
...
export function LRNBalanceWidget({
  ...
  return (
    <div className={`lrn-widget lrn-widget--${size} ${className}`.trim()}>
      {size === "sm" && <SmWidget data={data} animatedBalance={animatedBalance} />}
      {size === "md" && <MdWidget data={data} animatedBalance={animatedBalance} />}
      {size === "lg" && <LgWidget data={data} animatedBalance={animatedBalance} />}
