export function parseError(error: unknown): string {
    ...
    if (lowerError.includes("insufficient balance") || lowerError.includes("not enough") || lowerError.includes("underfunded")) {
        return "You don't have enough LRN for this action";
    }

    // Catch network mismatch issues from wallet
    if (lowerError.includes("network") || lowerError.includes("wrong network") || lowerError.includes("testnet") || lowerError.includes("mismatch")) {
        return "Please switch to Testnet in your wallet";
    }

    // Catch user rejection / cancellation
    if (lowerError.includes("user rejected") || lowerError.includes("cancelled") || lowerError.includes("rejected by user") || lowerError.includes("declined")) {
